...
// show consulted details
router.post('/showPatientID', (req, res) => {
    const { showPatientID } = req.body
    // search for information
    let show = `select *from records where opd_id = '${showPatientID}'`
    db.query(show, (err, showed) => {
        if(err) throw err
        req.flash('show', showed)
        res.redirect('/record')
    })
})
        ...
        if(checked[0].Lab_fee) {
            ...
            db.query(tests, (err, lname) => {
                ...
                if(lname.length == 0) {
                    ...
                    if(lname[0].Labtests == 'NA'){
                        req.flash('id', 'NO Tests')
                        res.redirect('/labfee')
                    } else {
                        let lab = '', i, labname;
                    let previousIndex = 0; 
...
router.post('/laboratory', (req, res) => {
    const { date } = req.body
    //get lab id's
    let info = `select p_id, records.opd_id, doa,Lab_fee, Labreports from records,bill where doa = '${date}' and p_doa = '${date}';`
    db.query(info, (err, results) => {
        if(err) throw err
        var n = results.length
        if(n == 0) {
...
//InPatient Discharge Handle
var in_PatientID_dis
router.post('/discharge', (req, res) => {
    in_PatientID_dis = req.body.inPatientID
    //discharge info
    let discharge = `select doc, status from in_patient_records where ipd_id = '${in_PatientID_dis}' and status = 'Active'`
    db.query(discharge, (err, dis) => {
        if(err) throw err
        if(dis.length == 0) {
            req.flash('id', 'Please Check information')
            res.redirect('/discharge')
        }else {
            //patient info 
            let sql = `select p_name from patient_info where p_id =(select p_id from in_patient_records where ipd_id = '${in_PatientID_dis}')`
            let yahoo = []
            db.query(sql, (err, Sql) => {
                if(err) throw err
                yahoo.push(Sql)
                yahoo.push(dis)
                req.flash('yahoo', yahoo)
                res.redirect('/discharge')
            })
        }
    })
...
//discharge entry
router.post('/indischarge', (req, res) => {
    const { date } = req.body
    //update
    let update = `update in_patient_records set discharge = '${date}', status = "Inactive", charges = 'Rs. 500' where ipd_id = '${in_PatientID_dis}';  update beds set status = "Inactive" where room_no = (select room_no from in_patient_records where ipd_id = '${in_PatientID_dis}') and bed_no = (select bed_no from in_patient_records where ipd_id = '${in_PatientID_dis}')`
    db.query(update, (err, up) => {
        if(err) throw err
        req.flash('P_success_msg', 'Thank You..!!')
        res.redirect('/discharge')
    })
})
